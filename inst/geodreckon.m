...
% with scalar arguments.
  ...
  narginchk(4, 6)
  if isequal(size(lat1), size(lon1), size(azi1), size(s12_a12))
    ...
    S1 = size(lat1); Z1 = -zeros(S1); % The size of geodesic line prob
    S = size(Z1); Z = -zeros(S); % The size of the full prob
  else
    error('lat1, lon1, s12_a12, azi1 have incompatible sizes')
  end
  if nargin <= 4
